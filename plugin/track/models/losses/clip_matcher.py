...
class ClipMatcher(nn.Module):
    ...
    # adds kwargs to allow custom loss interface e.g. tf loss
    def match_for_single_frame(self, outputs: dict, dec_lvl: int, if_step=False, **kwargs):
        ...
        def match_for_single_decoder_layer(unmatched_outputs, matcher):
            ...
            src_idx, tgt_idx = matcher.assign(
                bbox_pred, cls_pred, gt_bboxes, gt_labels)
            if src_idx is None:
                return None
            # concat src and tgt.
        ...
        for loss in self.losses:
            ...
            self.losses_dict.update(
                {'frame_{}_track_{}_{}'.format(self._current_frame_idx, key, dec_lvl): value for key, value in new_track_loss.items()})
    ...
    def prediction_loss(self, track_instances, predictions):
        ...
        for i in range(self._current_frame_idx, len(self.gt_instances)):
            # gt instances of i-th image.
            gt_instances_i = self.gt_instances[i]
