...
def random_drop_tracks(track_instances: Instances, drop_probability: float) -> Instances:
    if drop_probability > 0 and len(track_instances) > 0:
        ...
        dropped_track_instances = track_instances[keep_idxes]
    if len(dropped_track_instances) == 0:
        dropped_track_instances = track_instances
    return dropped_track_instances
...
class QueryInteractionModule(QueryInteractionBase):
    ...
    def _update_track_embedding(self, track_instances: Instances) -> Instances:
        ...
        # attention
        tgt = out_embed
        tgt2 = self.self_attn(q[:, None], k[:, None],
                              value=tgt[:, None])[0][:, 0]
        ...
        if self.update_query_pos:
            # ffn: linear_pos2
            query_pos2 = self.linear_pos2(self.dropout_pos1(
                self.activation(self.linear_pos1(tgt))))
            query_pos = query_pos + self.dropout_pos2(query_pos2)
            query_pos = self.norm_pos(query_pos)
            track_instances.query[:, :dim // 2] = query_pos

        query_feat2 = self.linear_feat2(self.dropout_feat1(
            self.activation(self.linear_feat1(tgt))))
