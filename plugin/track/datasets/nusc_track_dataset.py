...
class CustomTrackingEval(TrackingEval):

    # custom eval to be a bit less verbose than orig nusc version
    def main(self, render_curves: bool = True):
        """
...
class NuScenesTrackDataset(Dataset):
    ...
    def _get_sample_range(self, start_idx):
        ...
        assert self.sample_mode in [
            'fixed_interval', 'random_interval'], 'invalid sample mode: {}'.format(self.sample_mode)
        if self.sample_mode == 'fixed_interval':
            sample_interval = self.sample_interval
        elif self.sample_mode == 'random_interval':
            sample_interval = np.random.randint(1, self.sample_interval + 1)
        default_range = start_idx, start_idx + \
            (self.num_frames_per_sample - 1) * \
    ...
    def prepare_train_data(self, index):
        ...
        for i in range(start, end, interval):
            ...
            if data_i is None:
                return None
            if scene_token:
                if not scene_token == data_i['scene_token']:
                    return None
            else:
                scene_token = data_i['scene_token']
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        mmcv.dump(nusc_submissions, res_path, indent=4)
        return res_path
        ...
        if isinstance(result_files, dict):
            ...
            for name in result_names:
                ...
                if self.evaluate_detection:
                    ret_dict.update(
                        self._evaluate_single_detection(result_files[name]))
...
def output_to_nusc_box(detection):
    ...
    if 'track_ids' in detection.keys() and detection['track_ids'] is not None:
        track_ids = detection['track_ids'].numpy()
    else:
...
def _test():
    ...
    from plugin.track.datasets.pipelines.mutr_pipelines import FormatBundle3DTrack
