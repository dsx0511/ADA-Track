...
class FormatBundle3DTrack(DefaultFormatBundle):
    ...
    def __call__(self, results):
        ...
        if 'instance_inds' in results:
            instance_inds = [torch.tensor(_t)
                             for _t in results['instance_inds']]
            results['instance_inds'] = DC(instance_inds)

        keys = ['l2g_r_mat', 'l2g_t']
        for key in keys:
            if key in results:
                results[key] = DC(torch.tensor(
                    np.asarray(results[key]), dtype=torch.float))

        # TODO remove (legacy)
        # if 'sample_idx' in results:
        #     results['sample_idx'] = DC(
        #         data=results['sample_idx'], cpu_only=True)
        # if 'scene_token' in results:
        #     results['scene_token'] = DC(
        #         data=results['scene_token'], cpu_only=True)
...
@PIPELINES.register_module()
class FormatBundle3DDetectorBoxes(DefaultFormatBundle):
    """Default formatting bundle.
    ...
    def __init__(self):
        super(FormatBundle3DDetectorBoxes, self).__init__()
    ...
    def __call__(self, results):
        ...
        for key in [
                'lidar_t2_T_lidar_t1', 'obj_t1m_T_obj_t1', 'correct_obj_t2_T_obj_t1', 'query_feats_t1', 'query_pos',
                'ref_pts', 'ref_base_size', 'gt_boxes', 'pred_boxes', 'pred_logits',
                'pred_obj_idx', 'gt_obj_idx', 'timestamp'
        ]:
            if key not in results:
                continue
            if isinstance(results[key], list):
        ...
        for key in ['sample_idx', 'scene_token']:
            if key in results:
                results[key] = DC(
                    data=results[key], cpu_only=True)
...
class ScaleMultiViewImage3D(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            result_img = [mmcv.imresize(
                img, (x_size, y_size), return_scale=False) for img in results[key]]
