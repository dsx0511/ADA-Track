...
def _fill_trainval_infos(nusc,
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        # radar
        radar_names = ['RADAR_FRONT', 'RADAR_FRONT_LEFT',
        ...
        for radar_name in radar_names:
            ...
            while len(sweeps) < 5:
                if not radar_rec['prev'] == '':
                    radar_path, _, radar_intrin = nusc.get_sample_data(
                        radar_token)
        ...
        if not test:
            ...
            # add instance_ids
            instance_inds = [nusc.getind(
                'instance', ann['instance_token']) for ann in annotations]
...
if __name__ == '__main__':
    ...
    create_nuscenes_infos('data/nuscenes', 'ada_track')

    # generate .pkl for test set
    create_nuscenes_infos('data/nuscenes', 'ada_track', version='v1.0-test')
