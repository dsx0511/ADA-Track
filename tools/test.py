...
def main():
    ...
    if rank == 0:
        ...
        if args.out:
            ...
            pkl_filename = os.path.join(args.out, 'results.pkl')
            mmcv.dump(outputs, pkl_filename)
            kwargs.update({'jsonfile_prefix': args.out})
        if args.format_only:
        ...
        if args.eval:
            ...
            for k, v in eval_res.items():
                if "plot" in k:
                    # TODO refactor identification of plots
                    continue
                eval_res_filtered[k] = v
            print(eval_res_filtered)
